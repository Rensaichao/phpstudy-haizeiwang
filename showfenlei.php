 		...
 		<div class="clear"></div>
  		...
  		<div class="main">
				...
				<?php
				  ...
				  $rsfl = mysql_query("select * from tb_type order by id");
				  while($fl = mysql_fetch_row($rsfl)){
				?>
				  <a href="showfenlei.php?id=<?php echo $fl[0];?>"><?php echo $fl[1];?></a>
				<?php
				  }
				?>		
				...
				<?php
				  ...
				  $sqlid = mysql_query("select * from tb_type where id=".$id);
				  $infoid=mysql_fetch_array($sqlid);	  
				?>
				<div>本类角色 >><?php echo $infoid[typename];?></div>
				<?php
				  $pagesize = 2;
				  $sql = mysql_query("select * from tb_xuanshangling where typeid=". $id);
				  ...
				  if($total ==0){
					...
					$sql1 = "select * from tb_xuanshangling where typeid=".$id.
					" order by id desc limit ".(($page - 1)*$pagesize).",".$pagesize;
					$rs = mysql_query($sql1);
					$nur = ceil($total/$pagesize);
					echo "<a href='showfenlei.php?id=$id&page=1'><font face='webdings'>9</font></a>";
					if($page >= 2){
				?>
				      <a href="showfenlei.php?id=<?php echo $id?>&page=<?php echo $page-1;?>">
				  ...
				  ?>
				      <a href="showfenlei.php?id=<?php echo $id?>&page=<?php echo $nur;?>">
				 ...
				 ?>
				  ...
				  <?php
				      echo "<a href='showfenlei.php?id=$id&page=1'>首页</a>";
					  if($page >= 2){
				  ?>
				  <a href="showfenlei.php?id=<?php echo $id?>&page=<?php echo $page-1;?>">上一页</a>
				<?php
					}
					if($page < $nur){
				?>
				  <a href="showfenlei.php?id=<?php echo $id?>&page=<?php echo $page+1;?>">下一页</a>
				<?php
					}
				?>
				  <a href="showfenlei.php?id=<?php echo $id?>&page=<?php echo $nur;?>">尾页</a>
					
				<?php
				  }
				?>	
				<?php echo "<span>本站共有该类角色 " .$total."个 每页显示 "
				.$pagesize." 个 第 " .$page."页 共 " .$nur." 页</span>" ?>	
				</div>
