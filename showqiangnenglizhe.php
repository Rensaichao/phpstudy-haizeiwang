  		...
  		<div class="main">
			<div id="biaoti"><h2>强者世界</h2></div>
						<?php
						  ...
						  $pagesize = 2;
						  $sql = mysql_query("select * from tb_xuanshangling where typeid in('1','2')");
			    $total = mysql_num_rows($sql);
						  ...
						  $sql1 = "select * from tb_xuanshangling where typeid in('1','2') order by addtime desc limit "
						  .(($page - 1)*$pagesize).",".$pagesize;
						  $rs = mysql_query($sql1);
						  $nur = ceil($total/$pagesize);
						  echo "<a href='showqiangnenglizhe.php?page=1'>首页</a>";
						  if($page >= 2){
						?>
						<a href="showqiangnenglizhe.php?page=<?php echo $page-1;?>">上一页</a>
						<?php
						  }
						  if($page < $nur){
						?>
						<a href="showqiangnenglizhe.php?page=<?php echo $page+1;?>">下一页</a>
						<?php
						  }
						?>
						<a href="showqiangnenglizhe.php?page=<?php echo $nur;?>">尾页</a>
						<?php
						  echo "<div>";
